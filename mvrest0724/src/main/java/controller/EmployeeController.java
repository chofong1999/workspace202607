...
public class EmployeeController {
	 ...
	 private static final Map<Integer, Employee> DB = new HashMap<>();
	    ...
	    static {
	        DB.put(1, new Employee(nextId++, "Alice Chen",  "Engineering", 85000,"alice@demo.com",true));
	        DB.put(2, new Employee(nextId++, "Bob Wang",    "Marketing",   72000,"bob@demo.com",false));
	        DB.put(3, new Employee(nextId++, "Carol Liu",   "Engineering", 90000,"carol@demo.com",true));
	    }
    ...
    public Response searchDepartment(@DefaultValue("NA") @QueryParam("department")String department) {
    	   if(department.equals("NA"))
    		   return Response.status(Response.Status.BAD_REQUEST)
                       .entity("{\"message\":\"department 參數為必填\"}")
                       .build();
