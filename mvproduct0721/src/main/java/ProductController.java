...
public class ProductController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(path) {
		case "/test":
			List<Product> data2= listProduct(request,response);
			response.getWriter().append(data2.toString());
			break;
		case "/list":
//			List<Product> data= listProduct(request,response);
//			response.getWriter().append(data.toString()).append("\nend");
			List<Product> data= listProduct(request,response);
    ...
    private void deleteProduct(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
		  response.getWriter().append("delete completed");
		}catch(Exception ex) {}
    }
